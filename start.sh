...
[ -f "$SCRIPT_DIR/gateway/mvnw" ] || {
  err "gateway/mvnw not found"
  exit 1
}
log "mvnw found"
...
) > >(tee -a "$TOPO_LOG" | awk '{print "  '"${BLUE}"'[gatekeeper]'"${RESET}"' " $0}') 2>&1 &
TOPO_PID=$!
...
) > >(tee -a "$AI_LOG" | awk '{print "  '"${MAGENTA}"'[ai-service]'"${RESET}"' " $0}') 2>&1 &
AI_PID=$!
...
) > >(tee -a "$IMG_LOG" | awk '{print "  '"${CYAN}"'[image-gen]'"${RESET}"' " $0}') 2>&1 &
IMG_PID=$!
...
) > >(tee -a "$GW_LOG" | awk '{print "  '"${YELLOW}"'[gateway]'"${RESET}"' " $0}') 2>&1 &
GW_PID=$!
...
if [ "$SKIP_FRONTEND" = false ]; then
  ...
  (
    cd "$FE_DIR"
    exec npm run dev
  ) > >(tee -a "$FE_LOG" | awk '{print "  '"${GREEN}"'[frontend]'"${RESET}"' " $0}') 2>&1 &
  FE_PID=$!
