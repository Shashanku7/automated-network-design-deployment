...
const solutions = [
  {
    id: 'campus', icon: 'school', title: 'Campus Network',
    ...
    tags: ['Multi-building', 'Student Wi-Fi', 'Camera support', 'Guest network'],
  },
  {
    id: 'datacenter', icon: 'dns', title: 'Data Center',
    ...
    tags: ['High-speed backbone', 'Server racks', 'Redundancy', 'Storage network'],
  },
];
    ...
    <div className="p-8 max-w-5xl mx-auto">
      <header className="text-center mb-12 mt-8">
        <div className="flex items-center justify-center gap-2 text-primary mb-3">
          ...
          <span className="text-xs font-[family-name:var(--font-mono)] uppercase tracking-[0.2em]">New Design Wizard</span>
        </div>
        ...
        <p className="text-on-surface-variant mt-3 max-w-xl mx-auto">
          Select the type of network you need. We'll ask a few simple questions, then design the perfect solution.
        </p>
      </header>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {solutions.map(sol => (
          <button key={sol.id} onClick={() => handleSelect(sol.id)}
            ...
            <div className="w-16 h-16 rounded-xl bg-primary/10 flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">
              <span className="material-symbols-outlined text-primary text-3xl">{sol.icon}</span>
            </div>
            <h2 className="text-2xl font-bold font-[family-name:var(--font-headline)] text-on-surface group-hover:text-primary transition-colors">{sol.title}</h2>
            <p className="text-on-surface-variant text-sm mt-1">{sol.subtitle}</p>
            <p className="text-on-surface/70 text-sm mt-4 leading-relaxed">{sol.desc}</p>
            <div className="flex flex-wrap gap-2 mt-6">
              {sol.tags.map(t => <span key={t} className="text-xs bg-surface-container-high text-on-surface-variant px-3 py-1 rounded-full">{t}</span>)}
            </div>
