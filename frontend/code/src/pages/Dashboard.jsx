...
function ProjectListView() {
  ...
  const metrics = useMemo(() => [
    { label: 'Total Projects', value: projects.length.toString(), icon: 'folder_special', accent: 'primary' },
    { label: 'Active Deployments', value: projects.filter(p => p.status === 'deployed').length.toString(), icon: 'rocket_launch', accent: 'tertiary' },
    { label: 'In Design', value: projects.filter(p => p.status === 'designing' || p.status === 'draft').length.toString(), icon: 'hub', accent: 'primary' },
    { label: 'Complete', value: projects.filter(p => p.status === 'complete').length.toString(), icon: 'check_circle', accent: 'tertiary' },
  ], [projects]);

  if (projects.length === 0) {
    return (
      <div className="h-full flex flex-col items-center justify-center gap-6 p-8">
        <span className="material-symbols-outlined text-8xl text-outline/30">folder_special</span>
        <div className="text-center">
          <h2 className="text-2xl font-bold text-on-surface mb-2">No projects yet</h2>
          <p className="text-on-surface-variant mb-6">Create your first network design project to get started.</p>
  ...
  return (
    <div className="h-full overflow-y-auto p-8 space-y-8 custom-scrollbar">
      <section className="flex justify-between items-end">
        <div>
          <h1 className="text-3xl font-bold font-[family-name:var(--font-headline)] text-on-surface">Your Projects</h1>
          <p className="text-on-surface-variant mt-1">{projects.length} project{projects.length !== 1 ? 's' : ''} total</p>
        </div>
          ...
          <div className="bg-surface-container-low rounded-xl border border-outline-variant/15 overflow-hidden">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-outline-variant/5">
                {projects.map(p => (
                  <tr key={p.id} className="hover:bg-surface-container transition-colors cursor-pointer"
                    onClick={() => navigate(`/project/${p.id}`)}>
                    <td className="px-6 py-4">
                      <div className="font-medium text-on-surface">{p.title}</div>
                      <div className="text-xs text-outline font-[family-name:var(--font-mono)]">ID: {p.id.slice(0, 8)}</div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <span className={`text-sm font-medium px-2 py-0.5 rounded-full ${statusStyles[p.status] || statusStyles.draft}`}>
                          {statusLabel(p)}
                        </span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-sm text-on-surface-variant">{timeAgo(p.updatedAt)}</td>
                    <td className="px-6 py-4 text-right">
                      <button onClick={e => { e.stopPropagation(); navigate(`/project/${p.id}`); }}
...
function ProjectDashboardView({ projectId, meta }) {
  ...
  const status = state.workflowStatus === 'complete' ? 'Complete'
    : state.workflowStatus === 'running' || state.workflowStatus === 'awaiting_approval' ? 'In Progress'
    : state.requirements?.buildings?.length || state.requirements?.dcRacks ? 'Requirements Set'
    : state.solutionType ? 'Solution Chosen'
    : 'New';
  ...
  return (
    <div className="h-full overflow-y-auto p-8 space-y-8 custom-scrollbar">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: 'Design', path: `${projectId}/design`, icon: 'hub', desc: 'Run AI workflow', accent: 'primary' },
          { label: 'Bill of Materials', path: `${projectId}/bom`, icon: 'receipt_long', desc: 'View equipment list', accent: 'primary' },
          { label: 'Topology', path: `${projectId}/topology`, icon: 'schema', desc: 'Network diagrams', accent: 'primary' },
          { label: 'Deployment', path: `${projectId}/deployment`, icon: 'rocket_launch', desc: 'Push configs', accent: 'tertiary' },
        ].map(card => (
          <button key={card.label} onClick={() => navigate(`/project/${card.path}`)}
            ...
            <div className={`p-2 w-fit bg-${card.accent}/10 rounded-lg mb-4`}>
              <span className={`material-symbols-outlined text-${card.accent}`}>{card.icon}</span>
            </div>
            <div className="text-lg font-bold text-on-surface group-hover:text-primary transition-colors">{card.label}</div>
            <div className="text-sm text-on-surface-variant mt-1">{card.desc}</div>
          </button>
        ))}
      </section>
      ...
      <section className="pb-12">
        <div className="bg-surface-container-low rounded-xl border border-outline-variant/15 p-6">
          <h2 className="text-lg font-bold font-[family-name:var(--font-headline)] mb-4">Quick Actions</h2>
          <div className="flex flex-wrap gap-3">
            <button onClick={() => navigate(`/project/${projectId}/requirements`)}
              className="px-4 py-2 bg-primary text-on-primary font-bold rounded-lg hover:brightness-110 transition-all text-sm">
              Edit Requirements
            </button>
            <button onClick={() => navigate(`/project/${projectId}/design`)}
            ...
            {state.diagramUrl && (
              <button onClick={() => navigate(`/project/${projectId}/topology`)}
                className="px-4 py-2 border border-outline-variant/30 text-on-surface font-medium rounded-lg hover:bg-surface-container-high transition-all text-sm">
                View Topology
              </button>
            )}
