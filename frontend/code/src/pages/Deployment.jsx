      ...
      <div className="max-w-6xl mx-auto">
        <header className="mb-10">
          <div className="flex items-center gap-2 text-primary mb-2">
            ...
            <span className="text-xs font-[family-name:var(--font-mono)] uppercase tracking-[0.2em]">Final Step</span>
          </div>
          <h1 className="text-3xl font-bold font-[family-name:var(--font-headline)] text-on-surface tracking-tight">Review & Deploy</h1>
          <p className="text-on-surface-variant mt-2">Review your network design before finalizing.</p>
        </header>
      ...
      <div className="grid grid-cols-12 gap-8">
        ...
        <div className="col-span-12 lg:col-span-8 space-y-6">
              ...
              {bom.map((item, i) => (
                <div key={i} className="flex items-center gap-3 p-3 bg-surface rounded-lg">
                  ...
                  <div className="flex-1">
                    <div className="text-sm font-medium">{item.product} × {item.qty}</div>
                    <div className="text-xs text-on-surface-variant">{item.purpose}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div>
            <button onClick={() => setShowLogs(!showLogs)}
              ...
              <span className="material-symbols-outlined text-sm">{showLogs ? 'expand_less' : 'expand_more'}</span>
            ...
            {showLogs && (
              <div className="bg-surface-dim rounded-xl border border-outline-variant/10 p-4 font-[family-name:var(--font-mono)] text-xs leading-relaxed text-on-surface-variant max-h-48 overflow-y-auto">
                <p><span className="text-outline/50">[INFO]</span> <span className="text-primary-dim">Initializing deployment engine v2.4.0</span></p>
                <p><span className="text-outline/50">[INFO]</span> Connecting to network devices... <span className="text-tertiary">OK</span></p>
                <p><span className="text-outline/50">[INFO]</span> Validating configuration integrity...</p>
                <p><span className="text-outline/50">[INFO]</span> Waiting for administrator confirmation...</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="col-span-12 lg:col-span-4 space-y-6">
          ...
          <section className="bg-surface-container-low rounded-xl p-6 border border-outline-variant/15">
            ...
            <div className="space-y-3">
              {['Design Generated', 'Equipment Selected', 'Configuration Ready'].map(item => (
              ...
              <div className="flex items-center gap-3 p-2">
                <span className={`material-symbols-outlined text-sm ${confirmed ? 'text-tertiary' : 'text-outline'}`}
                  style={confirmed ? { fontVariationSettings: "'FILL' 1" } : {}}>
          ...
          <section className="bg-surface-container-highest rounded-xl p-6 border border-primary/20 relative overflow-hidden">
            ...
            <label className="flex items-start gap-3 cursor-pointer mb-6">
              ...
              <span className="text-sm text-on-surface-variant leading-relaxed">
                I confirm that the proposed design meets my requirements and I'm ready to proceed.
              </span>
            </label>
            <button onClick={handleDeploy} disabled={!confirmed || deploying || isComplete}
              ...
              {isComplete ? 'Deployment Complete!' : deploying ? 'Deploying...' : 'Execute Deployment'}
            </button>
          </section>
