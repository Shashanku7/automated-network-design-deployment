    ...
    <div className="h-full overflow-y-auto p-8 custom-scrollbar">
      <form onSubmit={handleSubmit} className="max-w-4xl mx-auto pb-24">
                    ...
                    <div key={b.id} className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-surface-container-low p-4 rounded-xl border border-outline-variant/20 items-end">
                      <div className="md:col-span-2 space-y-2">
                        ...
                        <input
                          ...
                          className={`w-full bg-surface-container-highest border rounded-lg px-4 py-2 text-on-surface focus:ring-1 focus:ring-primary outline-none transition-all ${errors[`b_${i}_name`] ? 'border-error' : 'border-outline-variant/30'
                            }`}
                        />
                      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              ...
              <button type="button" key={opt.val} onClick={() => set('uptimeLevel', opt.val)}
                className={`p-5 rounded-xl border text-left transition-all ${form.uptimeLevel === opt.val
        ...
        <Section title="Future plans" icon="trending_up">
          <div className="max-w-xs">
            <NumberInput
              label={isCampus ? "Do you expect your campus to grow? (Expected additional users/people)" : "Do you expect to add more capacity? (Expected additional server racks)"}
  ...
  return (
    <div className="space-y-2">
      ...
      <input type="number" min="0" value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder}
        className={`w-full bg-surface-container-low border rounded-lg px-4 py-3 text-on-surface placeholder:text-outline/50 focus:ring-1 focus:ring-primary focus:border-primary transition-colors ${error ? 'border-error/60 ring-1 ring-error/20' : 'border-outline-variant/30'
          }`} />
...
function ChipToggle({ label, active, onClick }) {
  return (
    <button type="button" onClick={onClick}
      className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${active ? 'bg-primary/20 text-primary border border-primary/40' : 'bg-surface-container-high text-on-surface-variant border border-transparent hover:border-outline-variant/30'
        }`}>
      {label}
    </button>
  );
...
function DeviceToggle({ icon, label, active, onClick }) {
  return (
    <button type="button" onClick={onClick}
      className={`flex flex-col items-center gap-2 p-4 rounded-xl border transition-all text-center ${active ? 'bg-primary/10 border-primary/40 text-primary' : 'bg-surface-container-low border-outline-variant/15 text-on-surface-variant hover:border-outline-variant/30'
        }`}>
...
function AreaToggle({ icon, label, active, onClick }) {
  return (
    <button type="button" onClick={onClick}
      className={`flex items-center gap-3 p-4 rounded-xl border transition-all text-left ${active ? 'bg-tertiary/10 border-tertiary/40' : 'bg-surface-container-low border-outline-variant/15 hover:border-outline-variant/30'
        }`}>
      <span className="text-2xl">{icon}</span>
